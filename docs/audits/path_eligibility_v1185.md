# Censo de elegibilidad de la critica archivada (D155) - OGR 0.1.185
...
| circular vuelcan | 0 |
| circular al borde | 13 |
| circular en reserva | 32 |
...
| no_circular | 0.1.185 | 37 |

De los 38 archivos no circulares, **1 no los escribio la version instalada**. 35 de 38 son de 0.1.173 y uno de 0.1.147: entre las instantaneas 0.1.180 y 0.1.181 son identicos byte a byte, de modo que «cuantas filas cambiaron» solo puede contestar «las dos mitades del 085» y lo contesta por aritmetica. Eso no es una medida de la busqueda: es que el lote no circular lleva once versiones sin re-correrse, y sale con numero propio.
...
`thrust_is_admissible` compara `sum(E_interior)` con cero, y el margen es esa suma dividida por `sum(|E_interior|)`: +1 con todas las caras en compresion, -1 con todas en traccion, y cerca de cero cuando el veredicto esta a punto de volcar. **13 filas medidas tienen |margen| < 0.05.** En el 085 ese numero paso de +0,044 a -0,627 con un cambio del 0,10 % en el factor.

| problema | archivo | metodo | margen | adm hoy | salida | inadm. archivadas |
|---|---|---|---|---|---|---|
| 093 | resultados_modelo_sin_conexion.json | gle_morgenstern_price | 0.000112 | True | reserva | 5290 |
...
| 090 | resultados.json | spencer | 0.001421 | True | reserva | 5715 |
| 093 | resultados_modelo_sin_conexion.json | spencer | 0.002203 | True | reserva | 4700 |
| 090 | resultados_modelo_sin_conexion.json | spencer | 0.024805 | True | reserva | 6664 |
...
| 093 | resultados.json | spencer | 0.029372 | True | reserva | 2058 |
...
| 085 | resultados_no_circular_pasivo.json | spencer | 0.1.185 | 2.071732 | 2.047170 | 1.1856 | True |
...
El censo de arriba mide el banco VIVO, y ahi el 085 no circular no puede volcar: sus archivos ya son los que escribio 0.1.181 y su critica si es admisible hoy. El vuelco solo se ve sobre la critica que la version ANTERIOR archivo, y eso es esta tabla (`censo_0.1.185_desde0.1.180.json`).  **3 de 229 filas medidas.**
