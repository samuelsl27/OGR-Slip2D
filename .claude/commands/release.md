...
1. Sube el número en los **ocho** sitios: `pyproject.toml`,
   `ogr_gui/main_window.py`, `ogr_slip2d/__init__.py`,
   `ogr_fem2d/__init__.py` y los `__version__` de `ogr_core/__init__.py`,
   `ogr_gui/__init__.py`, `ogr_cli/__init__.py` y `ogr_api/__init__.py`
   (este último desde v0.1.194). Los de `ogr_core`, `ogr_gui` y `ogr_cli`
   se quedaron olvidados en 0.1.1 hasta v0.1.59 justo por no estar en esta
   lista; `tests/test_version_consistency_v176.py` es la lista que manda. `ogr_data` sigue en 0.0.0 a propósito: es un stub previsto para
